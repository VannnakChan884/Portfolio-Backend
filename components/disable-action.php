...
function renderDisabledAction($text, $tooltip, $icon = null, $isButton = true)
{
    ...
    if ($isButton) {
        $html .= '<button disabled class="bg-gray-400 text-white px-4 py-2 rounded opacity-50 cursor-not-allowed flex items-center justify-end gap-2">';
        if ($icon) {
            $html .= "<i class='$icon'></i>";
        }
        $html .= htmlspecialchars($text) . '</button>';
    } else {
        $html .= '<button type="submit" class="pointer-events-none text-gray-400 cursor-not-allowed flex items-center justify-end gap-1">';
        if ($icon) {
            $html .= "<i class='$icon'></i>";
        }
        $html .= htmlspecialchars($text) . '</button>';
